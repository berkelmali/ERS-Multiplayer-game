/**
 * slapRules.js — THE single source of truth for ERS slap evaluation.
 *
 * Before v3.0.0 this file existed but only `tutorialMode.js` and the Cloud
 * Functions actually used it: `game.js::isValidSlap()` and
 * `firebaseSync.js::evaluateSlap()` each carried their own hand-written copy of
 * the same four rules. That was the COUNCIL.md Engineer's first `[Kesin]` risk —
 * add a rule to one, forget the other, and offline and online quietly disagree.
 * Those two duplicates are gone; both now call into this file.
 *
 * The registry also carries everything the two features built on top of it need:
 *
 *   • House Rules  — every rule is individually switchable in BOTH directions:
 *                    a table can drop Tens, or switch on Triple / Top-Bottom /
 *                    Four-in-a-Row, without touching any other file.
 *   • Forensics    — every rule knows how to describe its own NEAR MISS, which
 *                    is what lets the game answer "why wasn't that a slap?"
 *                    instead of just burning a card and staying silent.
 *
 * Rank encoding (matches game.js): 2–10 numeric, 11=J, 12=Q, 13=K, 14=A.
 *
 * ⚠️ `functions/slapRules.js` is a deploy-time mirror of this file (Cloud
 *    Functions cannot import from outside functions/). `npm test` FAILS if the
 *    two drift apart; `npm run sync:rules` regenerates the mirror.
 */
